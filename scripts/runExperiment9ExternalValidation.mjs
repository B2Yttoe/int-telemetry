...
export async function runExperiment9({
  ...
  const [modelSnapshot, validationSnapshot, validationRawRecords] = await Promise.all([
    readJson(modelSnapshotPath),
    readJson(validationSnapshotPath),
    readJson(validationRawPath),
  ]);
  const epochRecords = await Promise.all([
    buildEpochRecord({
      name: "CelesTrak Starlink 模型输入快照",
      url: modelSnapshot.source_url,
      path: modelSnapshotPath,
      role: "model-input",
      parserVersion: "int-temerity-real-tle-snapshot/v1",
  ...
  const validationEpoch = epochRecords.find((row) => row.role === "external-validation")?.epoch_end;
  const orbit = compareOrbitEpochs({ modelSnapshot, validationSnapshot: validationRawRecords, comparisonTime: validationEpoch });
  ...
  const manifest = {
    ...
    inputs: { model_snapshot: modelSnapshotPath, validation_snapshot: validationSnapshotPath, validation_raw: validationRawPath, external_report: existingExternalReportPath },
